...
# Estimate daily cases of Covid-19
covid_ny = N[-1] # ELIMINAMOS EL PRIMER VALOR, PUES NO SABEMOS CUANTOS HABIA EL DIA ANTERIOR (t_0)
names(covid_ny) = as.Date(nyc$date)

N_t0 = c(NA, nyc$deaths)
N_t1 = c(nyc$deaths, NA)
N = N_t1 - N_t0

# Estimate daily deaths
covid_ny_deaths = N[-1] # ELIMINAMOS EL PRIMER VALOR, PUES NO SABEMOS CUANTOS HABIA EL DIA ANTERIOR (t_0)
names(covid_ny_deaths) = as.Date(nyc$date)
...
new.deaths.uk = read.csv("https://data.london.gov.uk/download/coronavirus--covid-19--deaths/516cc869-179d-4cc7-84a4-2cf2fa2d112f/nhse_daily_announced_deaths_by_region.csv")

# Deaths with positive tests
covid_london_deaths = new.deaths.uk[grep("London", new.deaths.uk$nhs_england_region),]$new_deaths_with_positive_test
london_date_death = new.deaths.uk[grep("London", new.deaths.uk$nhs_england_region),]$date_of_death

X2 = aggregate(covid_london_deaths, list(london_date_death), sum)
...
br2 = aggregate(br$casos_novos, list(br$datahora), sum) # cases of Covid-19
br3 = aggregate(br$obitos_novos, list(br$datahora), sum) # deaths confimed with Covid-19

covid_saopaulo = br2$x
names(covid_saopaulo) = as.Date(br2$Group.1)

covid_saopaulo_deaths = br3$x
names(covid_saopaulo_deaths) = as.Date(br2$Group.1)
...
# DAILY DEATHS
# date = "FECHA_DEF
covid_cdmx_deaths = table(covid_cdmx$FECHA_DEF)
covid_cdmx_deaths = covid_cdmx_deaths[-grep("9999-99-99", names(covid_cdmx_deaths))]
...
M = list(cases_ny = covid_ny
         ...
         , deaths_ny = covid_ny_deaths
         , deaths_cdmx = covid_cdmx_deaths
         , deaths_london = deaths.london
         , deaths_saopaulo = covid_saopaulo_deaths)
